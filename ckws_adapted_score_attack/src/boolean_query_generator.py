...
class BooleanQueryResultExtractor(BooleanExtractor):
    """
    Keyword extractor augmented with a query generator for boolean queries.
    
    It corresponds to the index in the server. The fake queries are seen by the attacker.
    Generates queries from both conjunctive (AND) and disjunctive (OR) keyword combinations.
    """
    ...
    def get_fake_queries(self, size=1, hide_nb_files=True) -> Tuple[tf.Tensor, tf.Tensor]:
        """
        Generate fake boolean queries (both conjunctive and disjunctive).
        
        Args:
            size: Number of queries to generate
            hide_nb_files: If True, remove documents that don't match any query
            
        Returns:
            Tuple of (query_array, query_vocabulary)
            - query_array: Occurrence array for the queries
            - query_vocabulary: Keywords for the queries (with & for AND, | for OR)
        """
        logger.info("Generating fake boolean queries")
